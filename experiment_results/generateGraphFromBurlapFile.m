...
function generateGraphFromBurlapFile(folder,initTrial,endTrial,useMarkers)
...
%   endTrial: last experiment Trial
    ...
    for i=1:length(metricNames)
        ...
        handlePlots = plotMetric(episodes,metrics,metricNames{i},i,useMarkers);
...
% Returns the content read in experiment files
    ...
    for i = trials
       ...
       metricNames =  readF.colheaders(2:end);%strsplit(readF.textdata,';');
       %metricNames = metricNames(2:end-1);
...
function handlePlots = plotMetric(episodes,metrics,metricName,numberMetric,useMarker)
%DO last processing and plot

fewerParams = {'-r','-g','-b','-k','--c'};
params = {'-or','-+g','-*b','-^k','--dc'};
transparency = 0.3;
...
for i=1:numberAlg
   ...
   if useMarker
       ...
       [~,handlePlots(i)] = shadedplot(episodes{i}(1:maxIndex),maxProcessedMetric(1:maxIndex),minProcessedMetric(1:maxIndex),meanMetric(1:maxIndex),fewerParams{i},'k',transparency);
   end
